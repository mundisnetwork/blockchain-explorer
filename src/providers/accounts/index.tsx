...
import { PublicKey, Connection, StakeActivationData } from "@mundis/sdk";
...
export function useMintAccountInfo(
  ...
  return React.useMemo(() => {
    ...
    try {
      ...
      if (!data) return;
      if (data.program !== "token" || data.parsed.type !== "mint") {
        return;
      }
...
export function useTokenAccountInfo(
  ...
  try {
    const data = accountInfo?.data?.details?.data;
    if (!data) return;
    if (data.program !== "token" || data.parsed.type !== "account") {
      return;
    }
