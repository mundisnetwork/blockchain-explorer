...
export function lamportsToMundis(lamports: number | BN): number {
  if (typeof lamports === "number") {
    return Math.abs(lamports) / LAMPORTS_PER_MUNDIS;
  }
...
export function lamportsToMundisString(
  lamports: number | BN,
  maximumFractionDigits: number = 9
): string {
  const sol = lamportsToMundis(lamports);
  return new Intl.NumberFormat("en-US", { maximumFractionDigits }).format(sol);
}

export function MundisBalance({
  lamports,
  ...
  return (
    <span>
      ...
      <span className="font-monospace">
        {lamportsToMundisString(lamports, maximumFractionDigits)}
      </span>
    </span>
  );
}
